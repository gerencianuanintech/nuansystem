...
{
    public Task<SriDocumentMonitorSummary> GetSummaryAsync(long? importId=null,CancellationToken cancellationToken=default)=>
        apiClient.GetAsync<SriDocumentMonitorSummary>(
            "/api/sri/documents/monitor/summary"+BuildImportQuery(importId),
            cancellationToken);
    ...
    {
        var values=new Dictionary<string,string?> { ["importId"]=f.ImportId?.ToString(CultureInfo.InvariantCulture),["environment"]=f.Environment,["status"]=f.Status,["documentTypeCode"]=f.DocumentTypeCode,["sourceType"]=f.SourceType,["createdFrom"]=f.CreatedFrom?.ToString("O",CultureInfo.InvariantCulture),["createdTo"]=f.CreatedTo?.ToString("O",CultureInfo.InvariantCulture),["search"]=f.Search,["page"]=f.Page.ToString(CultureInfo.InvariantCulture),["pageSize"]=f.PageSize.ToString(CultureInfo.InvariantCulture) };
    ...
    private static string BuildImportQuery(long? importId) =>
        importId.HasValue
            ? "?importId="+importId.Value.ToString(CultureInfo.InvariantCulture)
            : string.Empty;
}
