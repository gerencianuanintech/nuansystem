...
public sealed partial class CostCentersForm : BaseGridCrudListForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.CostCenters;
    private readonly FinancialCatalogsViewModel viewModel;
    private readonly ApiSession session;

    public CostCentersForm()
    {
        viewModel = null!;
        session = null!;
        InitializeComponent();
    ...
    public CostCentersForm(FinancialCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new CostCenterEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new CostCenterEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK) return;
    ...
    protected override async Task CopyAsync()
    {
        if (SelectedItem() is not { } item) return;
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new CostCenterEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK) return;
        await viewModel.CreateAsync(form.Request);
