...
namespace NuanSystem.WinForms.Forms.FinancialCatalogs.BankAccountTypes;

public sealed partial class BankAccountTypesForm : BaseGridCrudListForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.BankAccountTypes;
    private readonly FinancialCatalogsViewModel viewModel;
    private readonly ApiSession session;

    public BankAccountTypesForm()
    {
        viewModel = null!;
        session = null!;
        InitializeComponent();
    ...
    public BankAccountTypesForm(FinancialCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new BankAccountTypeEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new BankAccountTypeEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
    ...
    protected override async Task CopyAsync()
    {
        ...
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new BankAccountTypeEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
