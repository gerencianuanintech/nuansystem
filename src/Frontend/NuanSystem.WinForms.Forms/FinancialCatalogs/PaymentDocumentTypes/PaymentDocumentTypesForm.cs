...
namespace NuanSystem.WinForms.Forms.FinancialCatalogs.PaymentDocumentTypes;

public sealed partial class PaymentDocumentTypesForm : BaseGridCrudListForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.PaymentDocumentTypes;
    private readonly FinancialCatalogsViewModel viewModel;
    private readonly ApiSession session;

    public PaymentDocumentTypesForm()
    {
        viewModel = null!;
        session = null!;
        InitializeComponent();
    ...
    public PaymentDocumentTypesForm(FinancialCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new PaymentDocumentTypeEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new PaymentDocumentTypeEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK) return;
    ...
    protected override async Task CopyAsync()
    {
        if (SelectedItem() is not { } item) return;
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new PaymentDocumentTypeEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK) return;
        await viewModel.CreateAsync(form.Request);
