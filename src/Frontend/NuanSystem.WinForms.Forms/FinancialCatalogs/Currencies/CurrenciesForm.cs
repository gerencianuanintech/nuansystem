...
public sealed partial class CurrenciesForm : BaseGridCrudListForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.Currencies;
    ...
    public CurrenciesForm(FinancialCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new CurrencyEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new CurrencyEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
    ...
    protected override async Task CopyAsync()
    {
        ...
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new CurrencyEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
