...
namespace NuanSystem.WinForms.Forms.FinancialCatalogs.BusinessLines;

public sealed partial class BusinessLinesForm : BaseGridCrudListForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.BusinessLines;
    private readonly FinancialCatalogsViewModel viewModel;
    private readonly ApiSession session;

    public BusinessLinesForm()
    {
        viewModel = null!;
        session = null!;
        InitializeComponent();
    ...
    public BusinessLinesForm(FinancialCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new BusinessLineEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new BusinessLineEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK) return;
    ...
    protected override async Task CopyAsync()
    {
        if (SelectedItem() is not { } item) return;
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new BusinessLineEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK) return;
        await viewModel.CreateAsync(form.Request);
