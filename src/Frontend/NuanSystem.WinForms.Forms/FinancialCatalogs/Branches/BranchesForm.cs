...
namespace NuanSystem.WinForms.Forms.FinancialCatalogs.Branches;

public sealed partial class BranchesForm : BaseGridCrudListForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.Branches;
    ...
    public BranchesForm(FinancialCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new BranchEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new BranchEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK) return;
    ...
    protected override async Task CopyAsync()
    {
        if (SelectedItem() is not { } item) return;
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new BranchEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK) return;
        await viewModel.CreateAsync(form.Request);
