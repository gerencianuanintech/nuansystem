...
public sealed partial class ApprovalFlowsForm : BaseGridCrudListForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.ApprovalFlows;
    private readonly FinancialCatalogsViewModel viewModel;
    private readonly ApiSession session;

    public ApprovalFlowsForm()
    {
        viewModel = null!;
        session = null!;
        InitializeComponent();
    ...
    public ApprovalFlowsForm(FinancialCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new ApprovalFlowEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new ApprovalFlowEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK) return;
    ...
    protected override async Task CopyAsync()
    {
        if (SelectedItem() is not { } item) return;
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new ApprovalFlowEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK) return;
        await viewModel.CreateAsync(form.Request);
