...
public sealed partial class ApprovalFlowEditForm : BaseEditForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.ApprovalFlows;

    public ApprovalFlowEditForm()
    {
        InitializeComponent();
        ConfigureForm();
    }

    public ApprovalFlowEditForm(FinancialCatalogItem item, bool copyMode = false) : this() => LoadCatalog(item, copyMode);
