...
namespace NuanSystem.WinForms.Forms.FinancialCatalogs.Projects;

public sealed partial class ProjectsForm : BaseGridCrudListForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.Projects;
    ...
    public ProjectsForm(FinancialCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new ProjectEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new ProjectEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK) return;
    ...
    protected override async Task CopyAsync()
    {
        if (SelectedItem() is not { } item) return;
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new ProjectEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK) return;
        await viewModel.CreateAsync(form.Request);
