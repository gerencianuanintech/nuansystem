...
namespace NuanSystem.WinForms.Forms.FinancialCatalogs.PaymentPriorities;

public sealed partial class PaymentPrioritiesForm : BaseGridCrudListForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.PaymentPriorities;
    private readonly FinancialCatalogsViewModel viewModel;
    private readonly ApiSession session;

    public PaymentPrioritiesForm()
    {
        viewModel = null!;
        session = null!;
        InitializeComponent();
    ...
    public PaymentPrioritiesForm(FinancialCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new PaymentPriorityEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new PaymentPriorityEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK) return;
    ...
    protected override async Task CopyAsync()
    {
        if (SelectedItem() is not { } item) return;
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new PaymentPriorityEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK) return;
        await viewModel.CreateAsync(form.Request);
