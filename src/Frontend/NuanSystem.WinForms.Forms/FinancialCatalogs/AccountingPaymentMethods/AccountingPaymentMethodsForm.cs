...
namespace NuanSystem.WinForms.Forms.FinancialCatalogs.AccountingPaymentMethods;

public sealed partial class AccountingPaymentMethodsForm : BaseGridCrudListForm
{
    private static readonly FinancialCatalogDescriptor Descriptor = FinancialCatalogDescriptors.AccountingPaymentMethods;
    private readonly FinancialCatalogsViewModel viewModel;
    private readonly ApiSession session;

    public AccountingPaymentMethodsForm()
    {
        viewModel = null!;
        session = null!;
        InitializeComponent();
    ...
    public AccountingPaymentMethodsForm(FinancialCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new AccountingPaymentMethodEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new AccountingPaymentMethodEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK) return;
    ...
    protected override async Task CopyAsync()
    {
        if (SelectedItem() is not { } item) return;
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new AccountingPaymentMethodEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK) return;
        await viewModel.CreateAsync(form.Request);
