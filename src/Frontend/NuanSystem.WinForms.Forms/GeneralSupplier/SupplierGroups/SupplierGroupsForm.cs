...
public sealed partial class SupplierGroupsForm : BaseGridCrudListForm
{
    ...
    public SupplierGroupsForm(
        GeneralSupplierCatalogsViewModel viewModel,
        ApiSession session,
    ...
    {
        using var form = new SupplierGroupEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new SupplierGroupEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
    ...
    protected override async Task CopyAsync()
    {
        ...
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new SupplierGroupEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
    ...
    private GeneralSupplierCatalogItem? SelectedItem()
    {
        return SelectedGridItem<GeneralSupplierCatalogItem>();
    }
