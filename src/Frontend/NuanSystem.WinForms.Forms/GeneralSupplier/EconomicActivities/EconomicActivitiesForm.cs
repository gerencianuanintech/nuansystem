...
public sealed partial class EconomicActivitiesForm : BaseGridCrudListForm
{
    ...
    public EconomicActivitiesForm(GeneralSupplierCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new EconomicActivityEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new EconomicActivityEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
    ...
    protected override async Task CopyAsync()
    {
        ...
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new EconomicActivityEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
