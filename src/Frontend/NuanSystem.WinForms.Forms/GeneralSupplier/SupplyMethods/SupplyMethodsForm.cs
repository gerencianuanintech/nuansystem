...
public sealed partial class SupplyMethodsForm : BaseGridCrudListForm
{
    ...
    public SupplyMethodsForm(GeneralSupplierCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new SupplyMethodEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new SupplyMethodEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
    ...
    protected override async Task CopyAsync()
    {
        ...
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new SupplyMethodEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
