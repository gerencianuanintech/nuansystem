...
public sealed partial class TaxRegimesForm : BaseGridCrudListForm
{
    ...
    public TaxRegimesForm(TaxCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new TaxRegimeEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new TaxRegimeEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
    ...
    protected override async Task CopyAsync()
    {
        ...
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new TaxRegimeEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
