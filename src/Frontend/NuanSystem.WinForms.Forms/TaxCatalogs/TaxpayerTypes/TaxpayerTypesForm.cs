...
namespace NuanSystem.WinForms.Forms.TaxCatalogs.TaxpayerTypes;

public sealed partial class TaxpayerTypesForm : BaseGridCrudListForm
{
    private static readonly TaxCatalogDescriptor Descriptor = TaxCatalogDescriptors.TaxpayerTypes;
    private readonly TaxCatalogsViewModel viewModel;
    private readonly ApiSession session;

    public TaxpayerTypesForm()
    {
        viewModel = null!;
        session = null!;
        InitializeComponent();
    ...
    public TaxpayerTypesForm(TaxCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new TaxpayerTypeEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new TaxpayerTypeEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
    ...
    protected override async Task CopyAsync()
    {
        ...
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new TaxpayerTypeEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK)
        {
            return;
        }
