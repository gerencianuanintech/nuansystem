...
public sealed partial class TaxSupportsForm : BaseGridCrudListForm
{
    private static readonly TaxCatalogDescriptor Descriptor = TaxCatalogDescriptors.TaxSupports;
    private readonly TaxCatalogsViewModel viewModel;
    private readonly ApiSession session;

    public TaxSupportsForm()
    {
        viewModel = null!;
        session = null!;
        InitializeComponent();
    ...
    public TaxSupportsForm(TaxCatalogsViewModel viewModel, ApiSession session, IGridColumnSettingsClient columnSettingsClient)
    {
    ...
    {
        using var form = new TaxSupportEditForm();
    ...
    protected override async Task EditAsync()
    {
        ...
        using var form = new TaxSupportEditForm(fullItem);
        if (form.ShowDialog(this) != DialogResult.OK) return;
    ...
    protected override async Task CopyAsync()
    {
        if (SelectedItem() is not { } item) return;
        var fullItem = await viewModel.GetByIdAsync(item.Id);
        using var form = new TaxSupportEditForm(fullItem, copyMode: true);
        if (form.ShowDialog(this) != DialogResult.OK) return;
        await viewModel.CreateAsync(form.Request);
