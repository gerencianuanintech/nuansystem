...
try
{
    var host = Host.CreateDefaultBuilder(args)
        ...
        .UseSerilog((context, services, configuration) => configuration.ReadFrom.Configuration(context.Configuration)
            ...
            .WriteTo.File("logs/nuansystem-sriworker-.log", rollingInterval: RollingInterval.Day, retainedFileCountLimit: 30, shared: true)
            .WriteTo.File("logs/nuansystem-sriworker-errors-.log", rollingInterval: RollingInterval.Day, retainedFileCountLimit: 30, shared: true,
        ...
        .ConfigureServices((context, services) =>
        {
            ...
            services.AddInfrastructureServices().AddPersistenceServices(context.Configuration);
...
{
    Log.Fatal(exception, "NuanSystem SRI Worker finalizo inesperadamente.");
