...
public sealed class UpdateSecurityFieldCommandHandler(ISecurityFieldRepository fieldRepository, ISecurityFormRepository formRepository)
    : ICommandHandler<UpdateSecurityFieldCommand, SecurityFieldDto>
{
    public async Task<Result<SecurityFieldDto>> Handle(UpdateSecurityFieldCommand request, CancellationToken cancellationToken)
    {
        if (await fieldRepository.GetByIdAsync(request.Id, cancellationToken) is null)
        {
            return Result<SecurityFieldDto>.Failure(
                "Campo no encontrado.",
                [new ApiError("SecurityFieldNotFound", "El campo no existe.", nameof(request.Id))]);
        }
        ...
        if (await fieldRepository.ExistsByCodeAsync(code, request.Id, cancellationToken))
        {
            return Result<SecurityFieldDto>.Failure(
        ...
        if (await fieldRepository.ExistsByFieldKeyAsync(request.FormId, fieldKey, request.Id, cancellationToken))
        {
            return Result<SecurityFieldDto>.Failure(
        ...
        await fieldRepository.UpdateAsync(new UpdateSecurityFieldData(
            request.Id,
            request.FormId,
            code,
            request.Name.Trim(),
        ...
        var field = await fieldRepository.GetByIdAsync(request.Id, cancellationToken)
            ?? throw new InvalidOperationException("El campo fue actualizado pero no pudo consultarse.");

        return Result<SecurityFieldDto>.Success(field, "Campo actualizado correctamente.");
    }
}
