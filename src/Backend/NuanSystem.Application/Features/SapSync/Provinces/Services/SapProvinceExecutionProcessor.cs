...
public sealed class SapProvinceExecutionProcessor(
    ...
    ISapSyncRetryPolicy retryPolicy) : ISapSyncScheduledExecutionProcessor
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var rows = await reader.GetProvincesAsync(
                context.CompanyId,
                cancellationToken);
                         ...
                         .Chunk(Math.Max(1, context.BatchSize)))
            {
                ...
                var snapshot = SapProvinceSnapshot.FromRecord(row);
                var snapshotJson = JsonSerializer.Serialize(snapshot, JsonOptions);
                var snapshotHash = SHA256.HashData(Encoding.UTF8.GetBytes(snapshotJson));
                var sourceKey = snapshot.ExternalCode;
                var startedAtUtc = DateTime.UtcNow;
                SapProvinceRecordProcessResult result;
                ...
                catch (Exception exception) when (exception is not OperationCanceledException)
                {
                    ...
                    result = new(
                        ...
                        decision.IsRetryable && !decision.MoveToDeadLetter
                            ? "Reintento programado para el provincia."
                            : "No fue posible procesar el provincia.");
                }
                ...
                var write = await executionRepository.UpsertDetailAsync(new(
                    ...
                    LocalEntityId: result.LocalProvinceId,
                    ...
                    ApprovedSnapshotType: SapSyncApprovedSnapshotTypes.ProvinceV1,
        ...
        catch (Exception exception)
        {
            ...
            if (current.Status == SapSyncExecutionStatuses.Running)
            {
                await TransitionAsync(
                    ...
                    "SAP_PROVINCE_EXECUTION_FAILED",
                    $"La ejecucion de provincias fallo: {exception.GetType().Name}.",
                    null,
        ...
        try
        {
            ...
            if (current?.Status == SapSyncExecutionStatuses.Running)
            {
                await TransitionAsync(
                    ...
                    results,
                    "SAP_PROVINCE_EXECUTION_INTERRUPTED",
                    "La ejecucion de provincias fue interrumpida de forma controlada.",
                    null,
                    timeout.Token);
    ...
    private static string? LastErrorMessage(IEnumerable<PersistedResult> results) =>
        LastErrorCode(results) is null ? null : "Uno o mas provincias no pudieron procesarse.";
