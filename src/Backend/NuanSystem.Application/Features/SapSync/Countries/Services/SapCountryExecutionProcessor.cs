...
public sealed class SapCountryExecutionProcessor(
    ...
    ISapSyncRetryPolicy retryPolicy) : ISapSyncScheduledExecutionProcessor
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var rows = await reader.GetCountriesAsync(
                context.CompanyId,
                cancellationToken);
                         ...
                         .Chunk(Math.Max(1, context.BatchSize)))
            {
                ...
                var snapshot = SapCountrySnapshot.FromRecord(row);
                var snapshotJson = JsonSerializer.Serialize(snapshot, JsonOptions);
                var snapshotHash = SHA256.HashData(Encoding.UTF8.GetBytes(snapshotJson));
                var sourceKey = Normalize(snapshot.CountryCode);
                var startedAtUtc = DateTime.UtcNow;
                SapCountryRecordProcessResult result;
                ...
                catch (Exception exception) when (exception is not OperationCanceledException)
                {
                    ...
                    result = new(
                        ...
                        decision.IsRetryable && !decision.MoveToDeadLetter
                            ? "Reintento programado para el pais."
                            : "No fue posible procesar el pais.");
                }
                ...
                var write = await executionRepository.UpsertDetailAsync(new(
                    ...
                    LocalEntityId: result.LocalCountryId,
                    ...
                    ApprovedSnapshotType: SapSyncApprovedSnapshotTypes.CountryV1,
        ...
        catch (Exception exception)
        {
            ...
            if (current.Status == SapSyncExecutionStatuses.Running)
            {
                await TransitionAsync(
                    ...
                    "SAP_COUNTRY_EXECUTION_FAILED",
                    $"La ejecucion de paises fallo: {exception.GetType().Name}.",
                    null,
        ...
        try
        {
            ...
            if (current?.Status == SapSyncExecutionStatuses.Running)
            {
                await TransitionAsync(
                    ...
                    results,
                    "SAP_COUNTRY_EXECUTION_INTERRUPTED",
                    "La ejecucion de paises fue interrumpida de forma controlada.",
                    null,
                    timeout.Token);
    ...
    private static string? LastErrorMessage(IEnumerable<PersistedResult> results) =>
        LastErrorCode(results) is null ? null : "Uno o mas paises no pudieron procesarse.";
