...
""";
        ...
        CancellationToken cancellationToken)
    {
        ...
        scriptPath = FindDatabaseScriptPath("119_master_sri_document_monitor_security.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("120_master_worker_heartbeat_operations.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }
        ...
        scriptPath = FindDatabaseScriptPath("062_master_tenant_configuration.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("063_tenant_global_ids_and_external_refs.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("064_master_sync_outbox_inbox.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("125_master_sync_outbox_promotion.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }
        ...
        scriptPath = FindDatabaseScriptPath("130_master_item_group_sync_registration.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("132_master_item_unit_of_measure_dependency.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("134_master_warehouse_sync_registration.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("137_master_currency_transactional_registration.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("139_master_sri_txt_import_security.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("152_master_sap_sync_profiles.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("154_master_sap_sync_profile_api_hardening.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("155_master_sap_sync_scheduler.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("156_master_sap_sync_scheduler_dapper_contract.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }
        ...
        scriptPath = FindDatabaseScriptPath("159_master_sap_warehouse_sync_capability.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("160_master_sap_sync_winforms_navigation.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }
        ...
        scriptPath = FindDatabaseScriptPath("163_master_carrier_transactional_registration.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("165_master_sap_sync_profile_inactive_capabilities.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }

        scriptPath = FindDatabaseScriptPath("166_master_sap_warehouse_profile_filter.sql");
        if (scriptPath is not null)
        {
            await ExecuteScriptFileAsync(connection, scriptPath, cancellationToken);
        }
