...
public sealed class ProductTypeSyncApplyRepository(ICompanyResolver companyResolver) : IProductTypeSyncApplyRepository
{
    private const string ApplyProcedure = "dbo.SP_NA_POST_PRODUCT_TYPE_SYNC_APPLY";

    public async Task<ProductTypeSyncApplyResult> ApplyAsync(int branchCompanyId,
        SyncEventApplyContext context, ProductTypeSyncPayload payload, SyncOperation operation,
        CancellationToken cancellationToken = default)
    {
        ...
        if (company.DatabaseEngine != DatabaseEngine.SqlServer)
            throw new NotSupportedException($"El motor {company.DatabaseEngine} no esta implementado para Sync ProductType.");
        ...
        try
        {
            ...
            var apply = await connection.QuerySingleAsync<ApplyRow>(new CommandDefinition(ApplyProcedure, new
            {
                ...
                NatureCode = Required(payload.NatureCode, nameof(payload.NatureCode), 30),
                payload.SortOrder,
                payload.IsSystem,
                IsActive = isActive,
                IsDeleted = isDeleted,
            ...
            {
                const string message = "El codigo de ProductType pertenece a otro GlobalId; no se realizo adopcion automatica.";
                await MarkDeadLetterAsync(connection, transaction, inboxId, message, cancellationToken);
                await transaction.CommitAsync(cancellationToken);
                return new(false, false, true, null, message, "SYNC_PRODUCT_TYPE_CODE_CONFLICT");
            }
            if (apply.ResultCode <= 0)
                throw new InvalidOperationException("El procedimiento de sincronizacion ProductType no aplico el evento.");

            await MarkAppliedAsync(connection, transaction, inboxId, cancellationToken);
            await transaction.CommitAsync(cancellationToken);
            return new(true, false, false, apply.ProductTypeId,
                $"Tipo de producto sincronizado por GlobalId {payload.GlobalId}.");
        }
    ...
    {
        if (string.IsNullOrWhiteSpace(value)) throw new InvalidOperationException($"{field} es requerido para sincronizar ProductType.");
    ...
    private sealed class ApplyRow { public int ResultCode { get; set; } public int? ProductTypeId { get; set; } }
}
