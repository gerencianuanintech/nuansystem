...
public sealed class ItemBrandSyncApplyRepository(ICompanyResolver companyResolver) : IItemBrandSyncApplyRepository
{
    private const string ApplyProcedure = "dbo.SP_NA_POST_ITEM_BRAND_SYNC_APPLY";

    public async Task<ItemBrandSyncApplyResult> ApplyAsync(int branchCompanyId,
        SyncEventApplyContext context, ItemBrandSyncPayload payload, SyncOperation operation,
        CancellationToken cancellationToken = default)
    {
        ...
        if (company.DatabaseEngine != DatabaseEngine.SqlServer)
            throw new NotSupportedException($"El motor {company.DatabaseEngine} no esta implementado para Sync ItemBrands.");
        ...
        try
        {
            ...
            {
                const string message = "El codigo de ItemBrand pertenece a otro GlobalId; no se realizo adopcion automatica.";
                await MarkDeadLetterAsync(connection, transaction, inboxId, message, cancellationToken);
                await transaction.CommitAsync(cancellationToken);
                return new(false, false, true, null, message, "SYNC_ITEM_BRAND_CODE_CONFLICT");
            }
            if (apply.ResultCode <= 0)
                throw new InvalidOperationException("El procedimiento de sincronizacion ItemBrand no aplico el evento.");

            await MarkAppliedAsync(connection, transaction, inboxId, cancellationToken);
            await transaction.CommitAsync(cancellationToken);
            return new(true, false, false, apply.ItemBrandId,
                $"Marca de articulos sincronizada por GlobalId {payload.GlobalId}.");
        }
    ...
    {
        if (string.IsNullOrWhiteSpace(value)) throw new InvalidOperationException($"{field} es requerido para sincronizar ItemBrands.");
    ...
    private sealed class ApplyRow { public int ResultCode { get; set; } public int? ItemBrandId { get; set; } }
}
