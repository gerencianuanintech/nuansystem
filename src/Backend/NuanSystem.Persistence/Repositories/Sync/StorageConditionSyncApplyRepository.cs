...
public sealed class StorageConditionSyncApplyRepository(ICompanyResolver companyResolver) : IStorageConditionSyncApplyRepository
{
 public async Task<StorageConditionSyncApplyResult> ApplyAsync(int branchCompanyId,SyncEventApplyContext context,StorageConditionSyncPayload payload,SyncOperation operation,CancellationToken cancellationToken=default)
 {
  ...
  if(company.DatabaseEngine!=DatabaseEngine.SqlServer) throw new NotSupportedException($"Motor {company.DatabaseEngine} no soportado para StorageCondition.");
  ...
  try
  {
   ...
   if(inbox?.Status==SyncEventStatus.DeadLetter.ToString()){await transaction.CommitAsync(cancellationToken);return new(false,false,true,null,inbox.LastErrorMessage??"Conflicto terminal.","SYNC_STORAGE_CONDITION_CODE_CONFLICT");}
 ...
 private sealed record Inbox(long Id,string Status,string? LastErrorMessage); private sealed class ApplyRow{public int ResultCode{get;set;}public int? StorageConditionId{get;set;}}
}
