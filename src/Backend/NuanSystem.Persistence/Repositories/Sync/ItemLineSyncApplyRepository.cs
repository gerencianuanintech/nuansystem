...
public sealed class ItemLineSyncApplyRepository(ICompanyResolver companyResolver) : IItemLineSyncApplyRepository
{
    private const string ApplyProcedure = "dbo.SP_NA_POST_ITEM_LINE_SYNC_APPLY";

    public async Task<ItemLineSyncApplyResult> ApplyAsync(int branchCompanyId,
        SyncEventApplyContext context, ItemLineSyncPayload payload, SyncOperation operation,
        CancellationToken cancellationToken = default)
    {
        ...
        if (company.DatabaseEngine != DatabaseEngine.SqlServer)
            throw new NotSupportedException($"El motor {company.DatabaseEngine} no esta implementado para Sync ItemLine.");
        ...
        try
        {
            ...
            {
                const string message = "El codigo de ItemLine pertenece a otro GlobalId; no se realizo adopcion automatica.";
                await MarkDeadLetterAsync(connection, transaction, inboxId, message, cancellationToken);
                await transaction.CommitAsync(cancellationToken);
                return new(false, false, true, null, message, "SYNC_ITEM_LINE_CODE_CONFLICT");
            }
            if (apply.ResultCode <= 0)
                throw new InvalidOperationException("El procedimiento de sincronizacion ItemLine no aplico el evento.");

            await MarkAppliedAsync(connection, transaction, inboxId, cancellationToken);
            await transaction.CommitAsync(cancellationToken);
            return new(true, false, false, apply.ItemLineId,
                $"Linea de articulos sincronizada por GlobalId {payload.GlobalId}.");
        }
    ...
    private static string Required(string value, string field, int max)
    {
        if (string.IsNullOrWhiteSpace(value))
            throw new InvalidOperationException($"{field} es requerido para sincronizar ItemLine.");
    ...
    private sealed class ApplyRow { public int ResultCode { get; set; } public int? ItemLineId { get; set; } }
}
