...
public sealed class ItemFamilySyncEventApplier(
    IItemFamilySyncApplyRepository repository) : ISyncEntityEventApplier
{
    private const string EntityName = SyncMasterBranchEntityCodes.ItemFamilies;
        ...
        CancellationToken cancellationToken = default)
    {
        if (context.TargetCompanyId is null)
        {
            return new SyncEventApplyResult(
                false,
                "ItemFamilies requiere sucursal destino.",
                ...
                cancellationToken))
        {
            return new SyncEventApplyResult(
                false,
                $"ItemGroup {payload.ItemGroupCode} todavia no existe en la sucursal.",
                "SYNC_ITEM_FAMILY_ITEM_GROUP_PENDING",
                Retryable: true);
        }
    ...
    private static ItemFamilySyncPayload ReadPayload(string payloadJson)
    {
        ...
        return payloadElement.Deserialize<ItemFamilySyncPayload>(JsonOptions)
            ?? throw new InvalidOperationException("Payload ItemFamilies no pudo ser deserializado.");
    }
}
