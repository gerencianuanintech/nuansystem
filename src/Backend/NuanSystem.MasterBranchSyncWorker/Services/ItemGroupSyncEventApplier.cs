...
public sealed class ItemGroupSyncEventApplier(
    IItemGroupSyncApplyRepository repository) : ISyncEntityEventApplier
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return new SyncEventApplyResult(false, "ItemGroups requiere sucursal destino.", "SYNC_TARGET_REQUIRED");
        }
    ...
    private static ItemGroupSyncPayload ReadPayload(string payloadJson)
    {
        ...
        return payloadElement.Deserialize<ItemGroupSyncPayload>(JsonOptions)
            ?? throw new InvalidOperationException("Payload ItemGroups no pudo ser deserializado.");
    }
}
