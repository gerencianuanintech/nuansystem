...
public sealed class CurrencySyncEventApplier(
    ICurrencySyncApplyRepository repository) : ISyncEntityEventApplier
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return new SyncEventApplyResult(false, "Currencies requiere sucursal destino.", "SYNC_TARGET_REQUIRED");
        }

        var payload = ReadPayload(context.PayloadJson);
        if (payload.GlobalId == Guid.Empty || payload.GlobalId != context.EntityGlobalId)
        {
            return new SyncEventApplyResult(false, "Payload Currencies no coincide con EntityGlobalId.", "SYNC_PAYLOAD_GLOBAL_ID_MISMATCH");
        }
    ...
    private static CurrencySyncPayload ReadPayload(string payloadJson)
    {
        ...
        return payloadElement.Deserialize<CurrencySyncPayload>(JsonOptions)
            ?? throw new InvalidOperationException("Payload Currencies no pudo ser deserializado.");
    }
}
