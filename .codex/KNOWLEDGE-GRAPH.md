...
The implemented vertical is rooted at `Application/Features/Carriers`, `Persistence/Repositories/CarrierRepository.cs`, `Api/Endpoints/CarrierEndpoints.cs`, tenant audit foundation `106`, tenant feature script `107`, Master security scripts `108`/`109`, and the frontend `Carriers` folders. It deliberately has no Domain entity, SAP mapping, synchronization event, outbox publisher, or BusinessPartners dependency.
