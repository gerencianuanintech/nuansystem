...
public sealed class SapCountryExecutionProcessorTests
{
    [Fact]
    public async Task FullExecution_TransientRowFailurePersistsApprovedSnapshotAndSchedulesRetry()
    {
        ...
        reader.GetCountriesAsync(context.CompanyId, Arg.Any<CancellationToken>())
        ...
        executionRepository.GetByExecutionUidAsync(context.ExecutionUid, Arg.Any<CancellationToken>())
            .Returns(running);
        geographyRepository.GetCountriesAsync(Arg.Any<CancellationToken>())
            .Returns<IReadOnlyCollection<CountryDto>>(_ => throw new TimeoutException("sensitive detail"));
        ...
        var processor = new SapCountryExecutionProcessor(
            reader,
            new SapCountryRecordProcessor(geographyRepository, Substitute.For<ISender>()),
            executionRepository,
            retryPolicy);

        await processor.ProcessAsync(context);

        await reader.Received(1).GetCountriesAsync(context.CompanyId, Arg.Any<CancellationToken>());
        await executionRepository.Received(1).UpsertDetailAsync(
            Arg.Is<SapSyncExecutionDetailData>(detail =>
                detail.SourceRecordKey == "EC"
                && detail.Status == SapSyncExecutionDetailStatuses.RetryScheduled
                && detail.ErrorClass == SapSyncSafeErrorClasses.Transient
                && detail.ApprovedSnapshotType == SapSyncApprovedSnapshotTypes.CountryV1
