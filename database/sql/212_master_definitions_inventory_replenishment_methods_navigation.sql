/* Migra seguridad/navegacion legacy de ReplenishmentMethods sin cambiar FormId ni accesos existentes. */
USE [NuanSystem_Master];
GO
SET NOCOUNT ON; SET XACT_ABORT ON;
IF DB_NAME()<>N'NuanSystem_Master' THROW 51212,'Migration 212 must run only in NuanSystem_Master.',1;
IF OBJECT_ID(N'dbo.MasterSchemaHistory',N'U') IS NULL THROW 51212,'MasterSchemaHistory is required.',1;
GO
BEGIN TRY
 BEGIN TRANSACTION;
 ...
 IF @ModuleId IS NULL OR @AdminRoleId IS NULL OR @ConfigurationMenuId IS NULL OR @DefinitionsMenuId IS NULL OR @InventoryMenuId IS NULL THROW 51212,'Required security parents are missing.',1;
 IF (SELECT COUNT(1) FROM dbo.SecurityForms WHERE (Code IN(N'FORM.GENERALINVENTORY.REPLENISHMENTMETHODS',N'FORM.DEFINITIONS.INVENTORY.REPLENISHMENTMETHODS') OR FormKey IN(N'inventory-replenishment-methods',N'replenishment-methods')) AND IsDeleted=0)>1 THROW 51212,'Multiple active forms identify ReplenishmentMethods.',1;
 IF (SELECT COUNT(1) FROM dbo.SecurityMenus WHERE (Code IN(N'MENU.GENERALINVENTORY.REPLENISHMENTMETHODS',N'MENU.DEFINITIONS.INVENTORY.REPLENISHMENTMETHODS') OR FormKey IN(N'inventory-replenishment-methods',N'replenishment-methods')) AND IsDeleted=0)>1 THROW 51212,'Multiple active menus identify ReplenishmentMethods.',1;

 DECLARE @Permissions table(Code nvarchar(120) PRIMARY KEY,Name nvarchar(160),Description nvarchar(300));
 INSERT @Permissions VALUES(N'GENERALINVENTORY.REPLENISHMENTMETHODS.READ',N'Ver metodos de reposicion',N'Consultar el maestro de metodos de reposicion.'),(N'GENERALINVENTORY.REPLENISHMENTMETHODS.MANAGE',N'Gestionar metodos de reposicion',N'Crear, editar y eliminar metodos de reposicion.');
 ...
 DECLARE @FormId int=(SELECT TOP(1) Id FROM dbo.SecurityForms WHERE (Code IN(N'FORM.GENERALINVENTORY.REPLENISHMENTMETHODS',N'FORM.DEFINITIONS.INVENTORY.REPLENISHMENTMETHODS') OR FormKey IN(N'inventory-replenishment-methods',N'replenishment-methods')) AND IsDeleted=0 ORDER BY CASE WHEN FormKey=N'replenishment-methods' THEN 0 ELSE 1 END);
 IF @FormId IS NULL BEGIN INSERT dbo.SecurityForms(Code,Name,Description,FormKey,FormType,IsVisible,IsActive,CreatedByUserName,CreatedAt) VALUES(N'FORM.GENERALINVENTORY.REPLENISHMENTMETHODS',N'Metodos de reposicion',N'Mantenimiento de metodos de reposicion',N'replenishment-methods',1,1,1,N'Sistema',SYSUTCDATETIME()); SET @FormId=CONVERT(int,SCOPE_IDENTITY()); END;
 UPDATE dbo.SecurityForms SET Code=N'FORM.GENERALINVENTORY.REPLENISHMENTMETHODS',Name=N'Metodos de reposicion',Description=N'Mantenimiento de metodos de reposicion',FormKey=N'replenishment-methods',FormType=1,IsVisible=1,IsActive=1,IsDeleted=0,DeletedByUserId=NULL,DeletedByUserName=NULL,DeletedAt=NULL,UpdatedByUserName=N'Sistema',UpdatedAt=SYSUTCDATETIME() WHERE Id=@FormId;
 ...
 IF @MenuId IS NULL BEGIN INSERT dbo.SecurityMenus(ParentId,Code,Name,Description,MenuType,FormId,FormKey,IconLarge,IconSmall,DisplayOrder,IsVisible,IsActive,CreatedByUserName,CreatedAt) VALUES(@InventoryMenuId,N'MENU.DEFINITIONS.INVENTORY.REPLENISHMENTMETHODS',N'Metodos de reposicion',N'Mantenimiento de metodos de reposicion',3,@FormId,N'replenishment-methods',N'Accordion/inventario_32.svg',N'Accordion/inventario_16.svg',120,1,1,N'Sistema',SYSUTCDATETIME()); SET @MenuId=CONVERT(int,SCOPE_IDENTITY()); END;
 UPDATE dbo.SecurityMenus SET ParentId=@InventoryMenuId,Code=N'MENU.DEFINITIONS.INVENTORY.REPLENISHMENTMETHODS',Name=N'Metodos de reposicion',Description=N'Mantenimiento de metodos de reposicion',MenuType=3,FormId=@FormId,FormKey=N'replenishment-methods',DisplayOrder=120,IsVisible=1,IsActive=1,IsDeleted=0,DeletedByUserId=NULL,DeletedByUserName=NULL,DeletedAt=NULL,UpdatedByUserName=N'Sistema',UpdatedAt=SYSUTCDATETIME() WHERE Id=@MenuId;
 ...
 IF NOT EXISTS(SELECT 1 FROM dbo.MasterSchemaHistory WHERE Version=N'20260813.212') INSERT dbo.MasterSchemaHistory(Version,Description) VALUES(N'20260813.212',N'Migra ReplenishmentMethods preservando formulario, menu y accesos legacy');
 COMMIT;
