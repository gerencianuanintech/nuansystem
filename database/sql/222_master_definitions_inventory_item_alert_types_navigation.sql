...
SET XACT_ABORT ON;
GO
IF DB_NAME()<>N'NuanSystem_Master' THROW 51222, 'Migration 222 must run only in NuanSystem_Master.', 1;
IF OBJECT_ID(N'dbo.MasterSchemaHistory',N'U') IS NULL THROW 51222, 'MasterSchemaHistory is required.', 1;
IF OBJECT_ID(N'dbo.SecurityFormOperations',N'U') IS NULL THROW 51222, 'SecurityFormOperations is required.', 1;
IF OBJECT_ID(N'dbo.SecurityRoleFormOperations',N'U') IS NULL THROW 51222, 'SecurityRoleFormOperations is required.', 1;
GO
BEGIN TRY
 BEGIN TRANSACTION;
 ...
 IF @ModuleId IS NULL OR @InventoryMenuId IS NULL OR @AdminRoleId IS NULL THROW 51222,'Required navigation parents are missing.',1;
 DECLARE @AlertTypesName nvarchar(160)=N'Tipos de alerta de art'+NCHAR(237)+N'culos';
 DECLARE @AlertTypesDescription nvarchar(300)=N'Mantenimiento de '+@AlertTypesName;
 DECLARE @Permissions table(Code nvarchar(120) PRIMARY KEY,Name nvarchar(160));
 INSERT @Permissions VALUES(N'GENERALINVENTORY.ITEMALERTTYPES.READ',N'Ver '+@AlertTypesName),(N'GENERALINVENTORY.ITEMALERTTYPES.MANAGE',N'Gestionar '+@AlertTypesName);
 ...
 DECLARE @FormId int=(SELECT TOP(1) Id FROM dbo.SecurityForms WHERE FormKey=N'item-alert-types' OR Code=N'FORM.DEFINITIONS.INVENTORY.ItemAlertTypes' ORDER BY IsDeleted,Id);
 IF @FormId IS NULL
 BEGIN
  ...
  VALUES(N'FORM.DEFINITIONS.INVENTORY.ItemAlertTypes',@AlertTypesName,@AlertTypesDescription,N'item-alert-types',1,1,1,1,1,N'Sistema',SYSUTCDATETIME());
  SET @FormId=CONVERT(int,SCOPE_IDENTITY());
 END;
 UPDATE dbo.SecurityForms SET Code=N'FORM.DEFINITIONS.INVENTORY.ItemAlertTypes',Name=@AlertTypesName,Description=@AlertTypesDescription,
  FormKey=N'item-alert-types',FormType=1,HasListView=1,HasEditView=1,IsVisible=1,IsActive=1,IsDeleted=0,
 ...
 DECLARE @MenuId int=(SELECT TOP(1) Id FROM dbo.SecurityMenus WHERE FormKey=N'item-alert-types' OR Code=N'MENU.DEFINITIONS.INVENTORY.ITEMALERTTYPES' ORDER BY IsDeleted,Id);
 IF @MenuId IS NULL
 BEGIN
  ...
  VALUES(@InventoryMenuId,N'MENU.DEFINITIONS.INVENTORY.ITEMALERTTYPES',@AlertTypesName,@AlertTypesDescription,3,@FormId,N'item-alert-types',90,1,1,N'Sistema',SYSUTCDATETIME());
  SET @MenuId=CONVERT(int,SCOPE_IDENTITY());
 END;
 UPDATE dbo.SecurityMenus SET ParentId=@InventoryMenuId,Code=N'MENU.DEFINITIONS.INVENTORY.ITEMALERTTYPES',Name=@AlertTypesName,Description=@AlertTypesDescription,
  MenuType=3,FormId=@FormId,FormKey=N'item-alert-types',DisplayOrder=90,IsVisible=1,IsActive=1,IsDeleted=0,
 ...
 IF (SELECT COUNT(*) FROM @ApplicableOperations)<>12 THROW 51222,'The twelve canonical CRUD operations are required.',1;
 ...
 IF NOT EXISTS(SELECT 1 FROM dbo.MasterSchemaHistory WHERE Version=N'20260814.222')
  INSERT dbo.MasterSchemaHistory(Version,Description) VALUES(N'20260814.222',N'Registers Tipos de alerta de artículos navigation and security');
 COMMIT;
END TRY
BEGIN CATCH
