...
SET XACT_ABORT ON;
GO
IF DB_NAME()<>N'NuanSystem_Master' THROW 51218, 'Migration 218 must run only in NuanSystem_Master.', 1;
IF OBJECT_ID(N'dbo.MasterSchemaHistory',N'U') IS NULL THROW 51218, 'MasterSchemaHistory is required.', 1;
IF OBJECT_ID(N'dbo.SecurityFormOperations',N'U') IS NULL THROW 51218, 'SecurityFormOperations is required.', 1;
IF OBJECT_ID(N'dbo.SecurityRoleFormOperations',N'U') IS NULL THROW 51218, 'SecurityRoleFormOperations is required.', 1;
GO
BEGIN TRY
 BEGIN TRANSACTION;
 ...
 DECLARE @FeatureName nvarchar(160)=N'Segmentos comerciales de art'+NCHAR(237)+N'culos';
 DECLARE @FeatureDescription nvarchar(240)=N'Mantenimiento de '+@FeatureName;
 IF @ModuleId IS NULL OR @InventoryMenuId IS NULL OR @AdminRoleId IS NULL THROW 51218,'Required navigation parents are missing.',1;
 DECLARE @Permissions table(Code nvarchar(120) PRIMARY KEY,Name nvarchar(160));
 INSERT @Permissions VALUES(N'GENERALINVENTORY.ITEMCOMMERCIALSEGMENTS.READ',N'Ver '+@FeatureName),(N'GENERALINVENTORY.ITEMCOMMERCIALSEGMENTS.MANAGE',N'Gestionar '+@FeatureName);
 ...
 DECLARE @FormId int=(SELECT TOP(1) Id FROM dbo.SecurityForms WHERE FormKey=N'item-commercial-segments' OR Code=N'FORM.DEFINITIONS.INVENTORY.ItemCommercialSegments' ORDER BY IsDeleted,Id);
 IF @FormId IS NULL
 BEGIN
  ...
  VALUES(N'FORM.DEFINITIONS.INVENTORY.ItemCommercialSegments',@FeatureName,@FeatureDescription,N'item-commercial-segments',1,1,1,1,1,N'Sistema',SYSUTCDATETIME());
  SET @FormId=CONVERT(int,SCOPE_IDENTITY());
 END;
 UPDATE dbo.SecurityForms SET Code=N'FORM.DEFINITIONS.INVENTORY.ItemCommercialSegments',Name=@FeatureName,Description=@FeatureDescription,
  FormKey=N'item-commercial-segments',FormType=1,HasListView=1,HasEditView=1,IsVisible=1,IsActive=1,IsDeleted=0,DeletedByUserId=NULL,DeletedByUserName=NULL,DeletedAt=NULL,
  UpdatedByUserName=N'Sistema',UpdatedAt=SYSUTCDATETIME() WHERE Id=@FormId;
 DECLARE @MenuId int=(SELECT TOP(1) Id FROM dbo.SecurityMenus WHERE FormKey=N'item-commercial-segments' OR Code=N'MENU.DEFINITIONS.INVENTORY.ITEMCOMMERCIALSEGMENTS' ORDER BY IsDeleted,Id);
 IF @MenuId IS NULL
 BEGIN
  ...
  VALUES(@InventoryMenuId,N'MENU.DEFINITIONS.INVENTORY.ITEMCOMMERCIALSEGMENTS',@FeatureName,@FeatureDescription,3,@FormId,N'item-commercial-segments',80,1,1,N'Sistema',SYSUTCDATETIME());
  SET @MenuId=CONVERT(int,SCOPE_IDENTITY());
 END;
 UPDATE dbo.SecurityMenus SET ParentId=@InventoryMenuId,Code=N'MENU.DEFINITIONS.INVENTORY.ITEMCOMMERCIALSEGMENTS',Name=@FeatureName,Description=@FeatureDescription,
  MenuType=3,FormId=@FormId,FormKey=N'item-commercial-segments',DisplayOrder=80,IsVisible=1,IsActive=1,IsDeleted=0,
 ...
 IF (SELECT COUNT(*) FROM @ApplicableOperations)<>12 THROW 51218,'The twelve canonical CRUD operations are required.',1;
 ...
 IF NOT EXISTS(SELECT 1 FROM dbo.MasterSchemaHistory WHERE Version=N'20260813.218')
  INSERT dbo.MasterSchemaHistory(Version,Description) VALUES(N'20260813.218',N'Registers ItemCommercialSegments navigation and security');
 UPDATE dbo.MasterSchemaHistory SET Description=N'Registers ItemCommercialSegments navigation and security' WHERE Version=N'20260813.218';
 COMMIT;
END TRY
BEGIN CATCH
