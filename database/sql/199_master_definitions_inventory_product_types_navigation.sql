/*
    Registra Tipos de producto en Configuracion > Definiciones > Inventario.
...
IF DB_NAME()<>N'NuanSystem_Master' THROW 51199,'Migration 199 must run only in NuanSystem_Master.',1;
IF OBJECT_ID(N'dbo.SecurityMenus',N'U') IS NULL OR OBJECT_ID(N'dbo.SecurityRoleMenus',N'U') IS NULL THROW 51199,'Security menus and role menus are required.',1;
IF OBJECT_ID(N'dbo.SecurityForms',N'U') IS NULL THROW 51199,'SecurityForms is required.',1;
IF OBJECT_ID(N'dbo.Permissions',N'U') IS NULL OR OBJECT_ID(N'dbo.RolePermissions',N'U') IS NULL THROW 51199,'API permission tables are required.',1;
IF OBJECT_ID(N'dbo.Modules',N'U') IS NULL OR OBJECT_ID(N'dbo.Roles',N'U') IS NULL THROW 51199,'Modules and Roles are required.',1;
IF OBJECT_ID(N'dbo.SecurityOperations',N'U') IS NULL OR OBJECT_ID(N'dbo.SecurityRoleFormOperations',N'U') IS NULL THROW 51199,'Form operation tables are required.',1;
IF OBJECT_ID(N'dbo.MasterSchemaHistory',N'U') IS NULL THROW 51199,'MasterSchemaHistory is required.',1;
GO
...
BEGIN TRY
 ...
 IF @ModuleId IS NULL OR @AdminRoleId IS NULL THROW 51199,'GENERALINVENTORY module and ADMIN role are required.',1;
 IF @ConfigurationMenuId IS NULL OR @DefinitionsMenuId IS NULL OR @InventoryMenuId IS NULL THROW 51199,'Configuration > Definitions > Inventory from migration 185 is required.',1;

 IF (SELECT COUNT(1) FROM dbo.SecurityForms WHERE (Code IN(N'FORM.GENERALINVENTORY.PRODUCTTYPES',N'FORM.DEFINITIONS.INVENTORY.PRODUCTTYPES') OR FormKey IN(N'inventory-product-types',N'product-types')) AND IsDeleted=0)>1
    THROW 51199,'Multiple active forms identify ProductTypes; reconcile before migration 199.',1;
 IF (SELECT COUNT(1) FROM dbo.SecurityMenus WHERE (Code IN(N'MENU.GENERALINVENTORY.PRODUCTTYPES',N'MENU.DEFINITIONS.INVENTORY.PRODUCTTYPES') OR FormKey IN(N'inventory-product-types',N'product-types')) AND IsDeleted=0)>1
    THROW 51199,'Multiple active menus identify ProductTypes; reconcile before migration 199.',1;

 DECLARE @Permissions table(Code nvarchar(120) PRIMARY KEY,Name nvarchar(160),Description nvarchar(300));
 INSERT @Permissions VALUES
 (N'GENERALINVENTORY.PRODUCTTYPES.READ',N'Ver tipos de producto',N'Consultar el maestro de tipos de producto.'),
 ...
 JOIN dbo.Permissions target ON target.Code=CASE legacyPermission.Code WHEN N'CATALOG.ITEMS.READ' THEN N'GENERALINVENTORY.PRODUCTTYPES.READ' WHEN N'CATALOG.ITEMS.MANAGE' THEN N'GENERALINVENTORY.PRODUCTTYPES.MANAGE' END
 ...
 DECLARE @FormId int=(SELECT TOP(1) Id FROM dbo.SecurityForms WHERE (Code IN(N'FORM.GENERALINVENTORY.PRODUCTTYPES',N'FORM.DEFINITIONS.INVENTORY.PRODUCTTYPES') OR FormKey IN(N'inventory-product-types',N'product-types')) AND IsDeleted=0 ORDER BY CASE WHEN FormKey=N'product-types' THEN 0 WHEN FormKey=N'inventory-product-types' THEN 1 ELSE 2 END);
 IF @FormId IS NULL
 BEGIN
  INSERT dbo.SecurityForms(Code,Name,Description,FormKey,FormType,IsVisible,IsActive,CreatedByUserName,CreatedAt)
  VALUES(N'FORM.GENERALINVENTORY.PRODUCTTYPES',N'Tipos de producto',N'Mantenimiento de tipos de producto',N'product-types',1,1,1,N'Sistema',SYSUTCDATETIME());
  SET @FormId=CONVERT(int,SCOPE_IDENTITY());
 END;
 UPDATE dbo.SecurityForms SET Code=N'FORM.GENERALINVENTORY.PRODUCTTYPES',Name=N'Tipos de producto',Description=N'Mantenimiento de tipos de producto',
  FormKey=N'product-types',FormType=1,IsVisible=1,IsActive=1,IsDeleted=0,DeletedByUserId=NULL,DeletedByUserName=NULL,DeletedAt=NULL,UpdatedByUserName=N'Sistema',UpdatedAt=SYSUTCDATETIME()
 WHERE Id=@FormId;

 DECLARE @MenuId int=(SELECT TOP(1) Id FROM dbo.SecurityMenus WHERE (Code IN(N'MENU.GENERALINVENTORY.PRODUCTTYPES',N'MENU.DEFINITIONS.INVENTORY.PRODUCTTYPES') OR FormKey IN(N'inventory-product-types',N'product-types')) AND IsDeleted=0 ORDER BY CASE WHEN FormKey=N'product-types' THEN 0 WHEN FormKey=N'inventory-product-types' THEN 1 ELSE 2 END);
 ...
 BEGIN
  ...
  VALUES(@InventoryMenuId,N'MENU.DEFINITIONS.INVENTORY.PRODUCTTYPES',N'Tipos de producto',N'Mantenimiento de tipos de producto',3,@FormId,N'product-types',
 ...
 END;
 UPDATE dbo.SecurityMenus SET ParentId=@InventoryMenuId,Code=N'MENU.DEFINITIONS.INVENTORY.PRODUCTTYPES',Name=N'Tipos de producto',Description=N'Mantenimiento de tipos de producto',
  MenuType=3,FormId=@FormId,FormKey=N'product-types',DisplayOrder=50,IsVisible=1,IsActive=1,IsDeleted=0,DeletedByUserId=NULL,DeletedByUserName=NULL,DeletedAt=NULL,
 ...
 IF NOT EXISTS(SELECT 1 FROM dbo.MasterSchemaHistory WHERE Version=N'20260812.199')
  INSERT dbo.MasterSchemaHistory(Version,Description) VALUES(N'20260812.199',N'Mueve Tipos de producto a Configuracion > Definiciones > Inventario');
 COMMIT;
END TRY
BEGIN CATCH
