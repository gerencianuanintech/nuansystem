/*
    Registra Provinces como capacidad operativa dependiente de Countries.
...
BEGIN
    ...
    SET Description = N'Catalogo geografico dependiente de Countries con productor incremental, fuente Full y aplicador idempotente por GlobalId.',
...
BEGIN
    ...
    SELECT
        company.Id, N'Provinces', CONVERT(bit, 0), N'MasterToBranch', N'MasterWins', 500, 3
    ...
    WHERE company.IsMaster = 1
      ...
      (
          ...
          WHERE existing.CompanyId = company.Id
            AND existing.EntityName = N'Provinces'
      );
END;
GO
...
BEGIN
    ...
    SELECT
        company.Id, N'Provinces', 0, 4, CONVERT(bit, 0)
    FROM dbo.Companies AS company
    WHERE company.IsMaster = 1
      AND NOT EXISTS
      (
...
IF OBJECT_ID(N'dbo.MasterSchemaHistory', N'U') IS NOT NULL
    AND NOT EXISTS (SELECT 1 FROM dbo.MasterSchemaHistory WHERE Version = N'20260716.086')
BEGIN
    INSERT INTO dbo.MasterSchemaHistory (Version, Description)
    VALUES (N'20260716.086', N'Provinces registrado como entidad operativa Maestro-Sucursal');
END;
GO
