/*
    Migration 219 - Forward repair for ItemCommercialSegments operation applicability.

    Target: NuanSystem_Master only.
    Prerequisites: migrations 179 and 218.
...
IF DB_NAME()<>N'NuanSystem_Master'
    THROW 51219,'Migration 219 must run only in NuanSystem_Master.',1;
IF OBJECT_ID(N'dbo.MasterSchemaHistory',N'U') IS NULL
    THROW 51219,'MasterSchemaHistory is required.',1;
IF OBJECT_ID(N'dbo.SecurityFormOperations',N'U') IS NULL
    THROW 51219,'Migration 179 is required before migration 219.',1;
IF OBJECT_ID(N'dbo.SecurityRoleFormOperations',N'U') IS NULL
    THROW 51219,'SecurityRoleFormOperations is required before migration 219.',1;
IF NOT EXISTS(SELECT 1 FROM dbo.MasterSchemaHistory WHERE Version=N'20260813.218')
    THROW 51219,'Migration 218 is required before migration 219.',1;
GO
...
BEGIN TRY
    ...
    IF @FormId IS NULL OR @AdminRoleId IS NULL
        THROW 51219,'The active ItemCommercialSegments form and ADMIN role are required.',1;
    ...
    IF (SELECT COUNT(*) FROM @ApplicableOperations)<>12
        THROW 51219,'The twelve canonical CRUD operations are required.',1;
    ...
    IF NOT EXISTS(SELECT 1 FROM dbo.MasterSchemaHistory WHERE Version=N'20260814.219')
        INSERT dbo.MasterSchemaHistory(Version,Description)
        VALUES(N'20260814.219',N'Repairs ItemCommercialSegments form operation applicability');
