...
- `228_tenant_business_partner_bidirectional_foundation.sql`: agrega identidad normalizada por rol, versiones, estado local, identidad global de hijos, destino/causalidad de LocalOutbox y conflictos auditables, sin activar capacidades. La normalización fija `Latin1_General_100_BIN2` tanto para el casing controlado como para almacenamiento/comparación binaria, evitando que la intercalación del tenant cambie la identidad o confunda valores Unicode distintos.
- `229_master_business_partner_bidirectional_governance.sql`: agrega la política central de códigos SAP para socios, causalidad en `SyncOutbox`, direcciones cerradas `MasterToBranch`/`BranchToMaster`, definiciones de propuesta/resultado inactivas, routing exacto hijo-padre/padre-hijo y permisos de conflictos concedidos inicialmente solo a `ADMIN`; no activa perfiles, destinos, flags ni workers.
